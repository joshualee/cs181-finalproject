...
class DataSet:
    ...
    def __init__(self, examples=None, attrs=None, target=-1, values=None,
        ...
        self.attrs = attrs if len(examples) > 0 else []
...
class DecisionTreeLearner(Learner):    
    ...
    def train(self, dataset):
        ...
        self.dt = self.decision_tree_learning(dataset.examples, dataset.inputs, depth=0)
    ...
    def decision_tree_learning(self, examples, attrs, default=None, depth=0):
        if len(examples) == 0:
        ...
        elif len(attrs) == 0 or depth == self.max_depth:
            return DecisionTree(DecisionTree.LEAF, classification=self.weighted_majority_value(examples))
        else:
            ...
            for (v, examples_i) in self.split_by(best, examples):
                subtree = self.decision_tree_learning(examples_i,
                  removeall(best, attrs), self.weighted_majority_value(examples), depth=depth+1)
                tree.add(v, subtree)
            return tree
    ...
    def weighted_majority_value(self, examples):
        """Return the most popular target value for this set of examples based on weight.
        (If target is binary, this is the majority; otherwise plurality.)"""
        g = self.dataset.target
        return argmax(self.dataset.values[g],
                      lambda v: self.weighted_count(g, v, examples))

    def count(self, attr, val, examples):
        return count_if(lambda e: e.attrs[attr] == val, examples)
    
    def weighted_count(self, attr, val, examples):
        return reduce(lambda acc, e: acc + e.weight if e.attrs[attr] == val else acc, examples, 0)
    
    def information_gain(self, attr, examples):
        def I(examples):
            target = self.dataset.target
            return information_content([self.weighted_count(target, v, examples)
